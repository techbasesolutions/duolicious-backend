...
def get_map_markers(s: t.SessionInfo):
    """All map markers for the current viewer, as individual points. Reads the
    viewer's existing search_cache (built by /search) -- read-only, does NOT
    rebuild it. The frontend clusters + spiderfies client-side."""
    if s.person_id is None:
        return '', 500
