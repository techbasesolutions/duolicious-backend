"""One-off: remind MEMBERS of their free Premium + tokens (beta).

Audience = ACTIVATED MEMBERS ONLY (the perks are on member accounts; the
waitlist has nothing to redeem). Dry-run is the default; `--send` is
required to actually send. Suppressed domains are skipped even on --send.

Run inside the api container on the droplet:

    python -m emails.send_beta_premium_reminder                 # DRY RUN
    python -m emails.send_beta_premium_reminder --only a@b.com  # send one
    python -m emails.send_beta_premium_reminder --send          # send to members
"""
...
from emails.beta_premium_reminder import beta_premium_html, SUBJECT, FROM_ADDR
