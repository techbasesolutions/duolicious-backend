...
from service.growth.queries import (count_newcomers_since, count_reinvite_cohort, dormant_cohort,
                                    newcomers_since, sought_gender_label)
...
def recipients() -> list[dict]:
    ...
    with api_tx('read committed') as tx:
        for row in dormant_cohort(tx, days=30, resend_days=30):
            ...
            # The count is already filtered to the people this member is
            # looking for (see _newcomer_predicate_sql), so the label just
            # names that group: "5 women", not "5 new members".
            label = sought_gender_label(tx, row['person_id'])
            out.append(dict(**row, newcomers=names, total_new=int(total), gender_label=label))
    return out
...
def build_for(row: dict) -> tuple[str, str]:
    ...
    return SUBJECT, reinvite_html(first, row.get('newcomers', []), row.get('total_new', 0), cta,
                                  ...
                                  gender_label=row.get('gender_label', 'new members'))

def preview_row(to: str) -> dict:
    return dict(person_id=0, email=to, name='Preview', newcomers=[dict(first_name='Rivka', country='GB')],
                total_new=5, gender_label='women')
